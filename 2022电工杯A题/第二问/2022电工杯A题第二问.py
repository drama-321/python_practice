...
def load_units_data():
    """加载机组参数数据"""
    return [
        {'name': '机组1', 'P_max': 600, 'P_min': 180, 'a': 0.226, 'b': 30.42, 'c': 786.80},
        {'name': '机组2', 'P_max': 300, 'P_min': 90, 'a': 0.588, 'b': 65.12, 'c': 451.32}
    ]
...
def main():
    ...
    wind_power = load_wind_data()  # 加载风电数据（300MW）
    ...
    for i, load_val in enumerate(load_demand):
        ...
        min_thermal_output = units[0]['P_min'] + units[1]['P_min']  # 机组1和机组2的最小出力总和
        ...
        # 输出有弃风的时间点
        if heavy_load > 0:
            print(f"在 {time_points[i]} 时，弃风量为 {heavy_load:.2f} MW")
    ...
    # 添加风电曲线
    plt.plot(wind_power, 'c-', label='风电出力 (0-300MW)')

    # 添加火电+风电总出力曲线
    total_power = [sum(x) for x in zip(P_results[0], P_results[1], wind_power)]
    plt.plot(total_power, 'm--', label='火电+风电总出力')

    plt.title('1号和2号机组与300MW风电日发电计划曲线', fontproperties=font, fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('第二问相关曲线图.png', dpi=300)
    plt.show()
    ...
    df = pd.DataFrame({
        ...
        '弃风量(MW)': heavy_loads,
        '火电+风电总出力(MW)': [sum(x) for x in zip(P_results[0], P_results[1], wind_power)],
        '功率平衡(MW)': power_balance
    })

    # 保存到Excel
    excel_path = '含风电的机组出力与功率平衡数据.xlsx'
    df.to_excel(excel_path, index=False)
    print(f"含风电的机组出力数据已保存到: {excel_path}")
