...
function [global_best_pos, global_best_val, convergence_curve] = pso_rastrigin(dimensions, num_particles, max_iter, bounds, w_start, w_end, c1, c2)
    ...
    % 计算每维需要的点数
    points_per_dim = ceil(num_particles^(1/dimensions));
    
    % 为每维生成均匀分布的点
    particles_pos = zeros(num_particles, dimensions);
    for d = 1:dimensions
        % 创建该维度的均匀分布点
        lin_points = linspace(bounds(1), bounds(2), num_particles);
        % 随机打乱这些点
        particles_pos(:, d) = lin_points(randperm(num_particles));
    end
